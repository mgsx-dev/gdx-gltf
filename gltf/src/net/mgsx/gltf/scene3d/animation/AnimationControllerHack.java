...
public class AnimationControllerHack extends AnimationController {
  ...
  private static Vector3 getTranslationAtTime(final NodeAnimation nodeAnim, final float time, final Vector3 out) {
  ...
  private static Quaternion getRotationAtTime(final NodeAnimation nodeAnim, final float time, final Quaternion out) {
  ...
  private static Vector3 getScalingAtTime(final NodeAnimation nodeAnim, final float time, final Vector3 out) {
  ...
  private static WeightVector getMorphTargetAtTime(final NodeAnimationHack nodeAnim, final float time, final WeightVector out) {
    ...
    if (nodeAnim.weights.size == 1) {
      return out.set(nodeAnim.weights.get(0).value);
    }

    int index = getFirstKeyframeIndexAtTime(nodeAnim.weights, time);

    Interpolation interpolation = nodeAnim.weightsMode;
  ...
  private static Transform getNodeAnimationTransform(final NodeAnimation nodeAnim, final float time) {
  ...
  private static void applyNodeAnimationBlending(final NodeAnimation nodeAnim, final ObjectMap<Node, Transform> out,
