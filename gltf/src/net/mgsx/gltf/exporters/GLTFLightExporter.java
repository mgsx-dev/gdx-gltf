...
class GLTFLightExporter {
  ...
  public void export(ObjectMap<Node, BaseLight<?>> lights) {
    ...
    for (Entry<Node, BaseLight<?>> entry : lights) {
      ...
      if (extLights == null) {
        extLights = new GLTFLights();
        base.root.extensions.set(KHRLightsPunctual.EXT, extLights);
      }
      ...
      if (nodeLight == null) {
        nodeLight = new GLTFLightNode();
        glNode.extensions.set(KHRLightsPunctual.EXT, nodeLight);
      }
