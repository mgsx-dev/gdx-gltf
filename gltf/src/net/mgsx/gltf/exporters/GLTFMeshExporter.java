...
class GLTFMeshExporter {
  ...
  private final ObjectMap<Mesh, GLTFPrimitive> layouts = new ObjectMap<>();
  ...
  private void exportMesh(GLTFPrimitive primitive, Mesh mesh) {
    ...
    Array<FloatBuffer> boneWeightsBuffers = new Array<>();
    Array<ShortBuffer> boneJointsBuffers = new Array<>(); // TODO short or byte (for small amount of bones)
    ...
    for (VertexAttribute a : mesh.getVertexAttributes()) {
      ...
      if (useTargets) {
        if (primitive.targets == null) primitive.targets = new Array<>();
