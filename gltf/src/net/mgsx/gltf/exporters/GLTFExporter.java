...
public class GLTFExporter {
  ...
  public void export(Array<SceneModel> scenes, SceneModel defaultScene, FileHandle file) {
    ...
    if (root.scene < 0) {
      throw new GLTFIllegalException("scene not found");
    }
    ...
    for (Node node : nodes) {
      ...
      if (node.parts.size > 0) {
        ...
        for (NodePart nodePart : node.parts) {
          ...
          if (materialIndex < 0) {
            throw new GLTFRuntimeException("material not found");
          }
  ...
  void useExtension(String ext, boolean required) {
    ...
    if (required) {
      if (root.extensionsRequired == null) {
        root.extensionsRequired = new Array<>();
      }
