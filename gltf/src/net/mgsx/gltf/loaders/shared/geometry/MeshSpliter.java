...
class MeshSpliter {
  ...
  public static void split(Array<float[]> splitVertices, Array<short[]> splitIndices, float[] vertices,
                           VertexAttributes attributes, int[] indices, int verticesPerPrimitive) {
    ...
    int maxIndexFound = 0;
    for (int k : indices) {
      maxIndexFound = Math.max(maxIndexFound, k);
    }

    IntMap<IntArray> groups = new IntMap<>();
