...
public class MeshLoader {
  ...
  private final ObjectMap<GLTFMesh, Array<NodePart>> meshMap = new ObjectMap<>();
  ...
  public void load(Node node, GLTFMesh glMesh, DataResolver dataResolver, MaterialLoader materialLoader) {
    ...
    if (parts == null) {
      ...
      for (GLTFPrimitive primitive : glMesh.primitives) {
        ...
        Array<GLTFAccessor> glAccessors = new Array<>();

        Array<int[]> bonesIndices = new Array<>();
        Array<float[]> bonesWeights = new Array<>();
        ...
        if (primitive.targets != null) {
          ...
          for (int unit = 0; unit < primitive.targets.size; unit++) {
            for (Entry<String, Integer> attribute : primitive.targets.get(unit)) {
              String attributeName = attribute.key;
              int accessorId = attribute.value;
              GLTFAccessor accessor = dataResolver.getAccessor(accessorId);
              glAccessors.add(accessor);
  ...
  private void generateParts(Node node, Array<NodePart> parts, Material material, String id, float[] vertices,
                             int vertexCount, short[] indices, VertexAttributes attributesGroup, int glPrimitiveType,
