...
import net.mgsx.gltf.data.data.*;
import net.mgsx.gltf.data.geometry.*;
import net.mgsx.gltf.loaders.blender.*;
import net.mgsx.gltf.loaders.exceptions.*;
import net.mgsx.gltf.loaders.shared.*;
import net.mgsx.gltf.loaders.shared.data.*;
import net.mgsx.gltf.loaders.shared.material.*;
...
public class MeshLoader {
				...
				if (primitive.indices != null) {
					...
					case GLTFTypes.C_UINT: {
						...
						for (int i = 0; i < groups; i++) {
							...
							generateParts(node, parts, material, glMesh.name, groupVertices, groupVertexCount, groupIndices,
								attributesGroup, glPrimitiveType, computeNormals, computeTangents, normalMapUVs);
						}
						Gdx.app.error("GLTF", "mesh split: " + parts.size + " meshes generated: " + totalVertices + " vertices, "
					...
					case GLTFTypes.C_SHORT: {
						...
						generateParts(node, parts, material, glMesh.name, vertices, maxVertices, indices, attributesGroup,
					...
					// non indexed mesh
					generateParts(node, parts, material, glMesh.name, vertices, maxVertices, null, attributesGroup, glPrimitiveType,
						computeNormals, computeTangents, normalMapUVs);
				}
			}
	...
	private void generateParts(Node node, Array<NodePart> parts, Material material, String id, float[] vertices, int vertexCount,
		short[] indices, VertexAttributes attributesGroup, int glPrimitiveType, boolean computeNormals, boolean computeTangents,
		VertexAttribute normalMapUVs) {
		...
		if (computeNormals || computeTangents) {
			...
			MeshTangentSpaceGenerator.computeTangentSpace(vertices, indices, attributesGroup, computeNormals, computeTangents,
				normalMapUVs);
		}
