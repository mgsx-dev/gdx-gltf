...
public class GLTFDemo extends ApplicationAdapter {
  ...
  private void createDefaultMaps() {
    if (alternateMaps != null) {
      diffuseCubemap = EnvironmentUtil.createCubemap(
        new InternalFileHandleResolver(),
        "textures/" + alternateMaps + "/diffuse/diffuse_",
        ".jpg",
      ...
      environmentCubemap = EnvironmentUtil.createCubemap(
        new InternalFileHandleResolver(),
        "textures/" + alternateMaps + "/environment/environment_",
        ".jpg",
        EnvironmentUtil.FACE_NAMES_NEG_POS
      );
      ...
      specularCubemap = EnvironmentUtil.createCubemap(
        new InternalFileHandleResolver(),
        "textures/" + alternateMaps + "/specular/specular_",
        "_",
        ".jpg",
        10,
        EnvironmentUtil.FACE_NAMES_NEG_POS
      ...
      environmentCubemap = EnvironmentUtil.createCubemap(
        new InternalFileHandleResolver(),
        "textures/environment/environment_",
        ".png",
        EnvironmentUtil.FACE_NAMES_NEG_POS
      );
  ...
  protected void loadIBL(FileHandle file) {
    ...
    try {
      newEnvironmentCubemap = EnvironmentUtil.createCubemap(
        new AbsoluteFileHandleResolver(),
        file.path() + "/environment/environment_",
        ".jpg",
        EnvironmentUtil.FACE_NAMES_NEG_POS
      );

      newDiffuseCubemap = EnvironmentUtil.createCubemap(
        new AbsoluteFileHandleResolver(),
        file.path() + "/diffuse/diffuse_",
        ".jpg",
        EnvironmentUtil.FACE_NAMES_NEG_POS
      );

      newSpecularCubemap = EnvironmentUtil.createCubemap(
        new AbsoluteFileHandleResolver(),
        file.path() + "/specular/specular_",
        "_",
        ".jpg",
        10,
        EnvironmentUtil.FACE_NAMES_NEG_POS
  ...
  protected void setShadow(boolean isOn) {
    if (isOn) {
      ...
      if (dla != null) {
        Array<BaseLight> lightsToRemove = new Array<>();
        Array<BaseLight> lightsToAdd = new Array<>();
    ...
    if (!outlineShaderValid) {
      ...
      if (ui.outlinesEnabled.isOn()) {
        ...
        if (ui.outlineDistFalloffOption.isOn()) {
          prefix += "#define DISTANCE_FALLOFF" + EOL;
        }
