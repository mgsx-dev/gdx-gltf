...
public class GLTFInspector {
  ...
  private final Array<BaseLight<?>> allLights = new Array<>();
  private final Array<Camera> allCameras = new Array<>();
  ...
  private void logScenes(SceneAsset asset) {
    ...
    int nTri = 0;
    int nRealTri = 0;
    ObjectSet<Material> mapSet = new ObjectSet<>();
    ...
    if (nMesh > 0) {
      log("Mesh", "count", nMesh, "parts", nMeshParts, "Vertices", nVert, "Tris", nTri, "Rendered", nRealTri);
    }
    if (nMaterials > 0) {
      log("Materials", "count", nMaterials);
    }
    if (asset.maxBones > 0) {
      log("Bones", "max", asset.maxBones, "count", nBones);
    }
  }
